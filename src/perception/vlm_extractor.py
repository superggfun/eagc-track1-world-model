...
class VLMExtractor:
    """Text and vision extractor backed by an OpenAI-compatible local Qwen/vLLM client.

    Supports text-only structured extraction and frame-based visual extraction when a
    local vision-capable endpoint is available.
    """
...
def normalize_objects(
    ...
    object_names = set()
    state_index: Dict[tuple[Any, Any], str] = {}
    for state in states:
        if not isinstance(state, dict):
            continue
        value = state.get("value")
        if value is not None:
            state_index.setdefault((state.get("entity"), state.get("attribute")), str(value))

    support_by_subject: Dict[Any, str] = {}
    for relation in relations:
        if not isinstance(relation, dict):
            continue
        if relation.get("relation") in {"on", "inside", "near"}:
            support_by_subject.setdefault(
    ...
    for item in objects:
        ...
        location_value = state_index.get((name, "location"))
        state_value = state_index.get((name, "state"))
        support = support_by_subject.get(name, "")
        location = normalize_location(
            obj.get("location") or location_value,
            room=str(obj.get("room") or "unknown"),
            support=support,
            confidence=float(obj.get("confidence", 0.75)),
        )
        state = obj.get("state") or state_value or "observed"
