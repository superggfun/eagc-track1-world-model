...
from perception.policy import should_run_post_action_vlm
...
class Track1ProcedureRunner:
    ...
    def __init__(
        ...
        self.perception_skip_count = 0
        self.perception_skip_reasons: Dict[str, int] = {}
        self.task = ""
        self.plan: Dict[str, Any] = {}
        self.evaluator_context = evaluator_context or {}

    def run_episode(self, episode_id: str) -> Dict[str, Any]:
        initial = self._reset_environment(episode_id)
        world_model = self.store.initialize(str(initial.get("episode_id") or episode_id))
    ...
    def task_reception_phase(self) -> None:
        packet = self._receive_task_packet()
    ...
    def _reset_environment(self, episode_id: str) -> Dict[str, Any]:
        episode_config = {"episode_id": episode_id, "reveal_task": False}
        try:
            signature = inspect.signature(self.env.reset)
            params = signature.parameters
            if "episode_config" in params or "config" in params:
                return self.env.reset(episode_config)
            if "reveal_task" in params:
                return self.env.reset(reveal_task=False)  # type: ignore[call-arg]
    ...
    def _receive_task_packet(self) -> Dict[str, Any]:
        reveal_task = getattr(self.env, "reveal_task", None)
        if callable(reveal_task):
            return reveal_task()
        packet = self.env.observe()
        if not isinstance(packet, dict):
            return {"task": "", "observation": str(packet)}
        packet.setdefault("task", packet.get("metadata", {}).get("task", "") if isinstance(packet.get("metadata"), dict) else "")
        return packet
    ...
    def audit_updates(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "perception_call_count": self.perception_call_count,
            "perception_skip_count": self.perception_skip_count,
            "vlm_call_saved_count": self.perception_skip_count,
            "perception_skip_reasons": dict(self.perception_skip_reasons),
        }
    ...
    def _execute_action(
        ...
        if isinstance(packet, dict):
            apply_environment_context(self.store.world_model, packet)
            self._refresh_discovery_fields(self.store.world_model)
            phase = "execution" if mode in ("executing", "resuming", "recovering") else mode
            should_perceive, reason = should_run_post_action_vlm(
                action, result, self.store.world_model, phase=phase, recovery=recovery,
            )
            if should_perceive:
                self._perceive(packet, self.task or "Explore the environment.")
            else:
                self._record_perception_skip(action, packet, reason)
        return result
    ...
    def sync_structured_observation_packet(
        self, world_model: Dict[str, Any], packet: Dict[str, Any]
    ) -> None:
        """Cheap structured sync — no VLM call.

        Updates agent state, current room, visible objects, environment
        context, and discovery fields from the structured observation
        packet returned by the environment.
        """
        apply_environment_context(world_model, packet)
        self._refresh_discovery_fields(world_model)

    def _record_perception_skip(
        self, action: str, packet: Dict[str, Any], reason: str
    ) -> None:
        self.perception_skip_count += 1
        self.perception_skip_reasons[reason] = (
            self.perception_skip_reasons.get(reason, 0) + 1
        )
        observation = packet.get("observation", "")
        self.logger.log(
            step=self.step,
            event_type="perception_skipped",
            observation=_render_observation(observation),
            action=action,
            result="skipped",
            notes=reason,
