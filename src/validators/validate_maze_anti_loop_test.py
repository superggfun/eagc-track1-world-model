...
def validate(status_path: Path) -> Dict[str, List[str]]:
    ...
    world_model_path = _resolve_status_path(status_path, status.get("world_model_path"), "world_model.json")
    episode_log_path = _resolve_status_path(status_path, status.get("episode_log_path"), "episode_log.jsonl")
    metrics_path = _resolve_status_path(status_path, status.get("maze_metrics_path"), "maze_metrics.json")
    report_path = _resolve_status_path(status_path, status.get("anti_loop_report_path"), "anti_loop_report.md")
    for label, path in [
...
def _resolve_status_path(status_path: Path, value: Any, default_name: str) -> Path:
    path = Path(str(value or default_name))
