...
def validate(world_model_path: Path, audit_path: Path, episode_log_path: Path) -> List[str]:
    ...
    errors.extend(_validate_score(audit, audit_path))
    return errors
...
def _validate_score(audit: Dict[str, Any], audit_path: Path) -> List[str]:
    score_path = _resolve_audit_path(str(audit.get("track1_score_path", "")), audit_path)
