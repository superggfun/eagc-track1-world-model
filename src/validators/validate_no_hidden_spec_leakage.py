...
def validate(world_model_path: Path, audit_path: Path, episode_log_path: Path) -> List[str]:
    ...
    if errors:
        return errors
    spec_path = _resolve_audit_path(str(audit.get("generated_episode_spec_path", "")), audit_path)
    ...
    agent_visible_paths = [
        world_model_path,
        episode_log_path,
        _resolve_audit_path(str(audit.get("qwen_response_summary_path", "")), audit_path),
        _resolve_audit_path(str(audit.get("output_dir", "")), audit_path) / "qwen_calls.jsonl",
    ]
