...
def validate(result_path: Path, audit_path: Path | None = None) -> List[str]:
    ...
    if audit:
        ...
        if expected_path:
            try:
                resolved_expected = _resolve_audit_path(expected_path, audit_path)
...
def _resolve_audit_path(value: str, audit_path: Path | None) -> Path:
    path = Path(value)
    if path.is_absolute() or audit_path is None:
        return path
