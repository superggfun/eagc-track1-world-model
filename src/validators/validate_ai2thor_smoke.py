...
def validate(world_model_path: Path, audit_path: Path) -> List[str]:
    ...
    frame_path = _resolve_audit_path(str(audit.get("simulator_frame_path", "")), audit_path)
    metadata_path = _resolve_audit_path(str(audit.get("simulator_metadata_path", "")), audit_path)
    episode_log_path = _resolve_audit_path(str(audit.get("episode_log_path", "")), audit_path)
...
def _resolve_audit_path(value: str, audit_path: Path) -> Path:
    path = Path(value)
    if path.is_absolute():
        return path
