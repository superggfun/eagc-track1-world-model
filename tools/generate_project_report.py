...
v0.9.1 finalized a real-image local visual sequence smoke validation using three Pexels bedroom frames stored locally as `frame_000.jpg`, `frame_001.jpg`, and `frame_002.jpg`.
...
The run validated object persistence across frames, `not_observed_current_frame` records for objects that disappear temporarily, and stale/active relation updates for changed spatial relations.

Object and relation counts can vary slightly across real Qwen vision runs, so the stable validation target is structured output plus temporal world-model consistency.
...
- a LocalSim Track 1 run with `world_model.json`, `episode_log.jsonl`, `run_audit.json`, `harness_result.json`, and `track1_score.json`
- a visual evidence run with `world_model.json`, `episode_log.jsonl`, `run_audit.json`, `harness_result.json`, `visual_task_result.json`, and `qwen_response_summary.json`

The source package is generated from git tracked files plus the harness refresh files when they are still untracked locally, and excludes outputs, local images, `.venv-ai2thor`, `source_pack`, zip files, and `__pycache__`.
