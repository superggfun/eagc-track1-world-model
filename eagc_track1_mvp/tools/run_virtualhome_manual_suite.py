...
STATUS_PATH = OUTPUT_DIR / "manual_suite_status.json"
...
def main() -> int:
    ...
    if not _is_port_open("127.0.0.1", 8080):
        ...
        print("VirtualHome manual-play tier skipped: 127.0.0.1:8080 is not listening.")
        return 0

    commands = [
        [sys.executable, "tools/check_virtualhome_env.py"],
        [sys.executable, "tools/test_virtualhome_windows_spike.py"],
    ...
    report["reason"] = "virtualhome_manual_suite_passed" if report["success"] else "virtualhome_manual_suite_failed"
    _write_report(report)
...
def _write_report(report: Dict[str, object]) -> None:
    ...
    print(f"VirtualHome manual suite report written to {STATUS_PATH}")
