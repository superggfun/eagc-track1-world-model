...
Core modules now live under `src/`. The project uses a flat `src` layout without a package name such as `eagc_track1_mvp`.

| Directory | Purpose |
|-----------|---------|
| `src/` | Core Python runtime modules (world_model, planner, executor, harness, etc.) |
| `tests/` | Automated tests |
| `tools/` | Packaging, report generation, and submission checks |
| `assets/` | Small demo/test assets |
| `docs/` | Development and method documentation |
| `submission_package/` | Source-side submission documents |
| `submission_bundle/` | Generated artifact bundle, excluded from source commits |
| `outputs/` | Local run outputs, excluded |
| `dist/` | Generated archives, excluded |

Harness commands:
```bash
python -m harness.run_track1 ...
python -m harness.run_visual_demo ...
python -m harness.validate_outputs ...
```

Tools run from the project root with `PYTHONPATH=src`:
```bash
PYTHONPATH=src python tools/create_submission_bundle.py
PYTHONPATH=src python tools/run_test_suite.py
```
...
The project does not claim success for reserved simulators. Stub adapters return graceful blockers rather than fake scene graphs, frames, or action success. The official hidden runtime boundary is documented in `docs/OFFICIAL_RUNTIME_ADAPTER.md`; `src/env_adapters/official_env.py` is fail-closed until organizer API details are released.
...
```powershell
# Report PDF generation (weasyprint + playwright)
pip install -r requirements-report.txt

# Optional simulator backends (ai2thor, habitat-sim, habitat-lab)
pip install -r requirements-optional-sim.txt
```
...
## Reproduce sample Track 1 outputs
...
For an offline deterministic smoke run, add `--mock`.
...
If local vLLM is not running, use the deterministic smoke path:

```powershell
python -m harness.run_visual_demo --frames assets/test_sequences/bedroom_sequence --output-dir outputs/visual_evidence_demo --validate --mock
```
...
```powershell
python tools/run_maze_stress_test.py --episode generated_grid_maze --seed 42 --difficulty medium --max-steps 200 --output-dir outputs/maze_stress
python -m validators.validate_maze_outputs --output-dir outputs/maze_stress --recursive
python tools/run_maze_anti_loop_test.py --episode all --output-dir outputs/maze_anti_loop
python -m validators.validate_maze_outputs --output-dir outputs/maze_anti_loop --recursive
python tools/run_test_suite.py --tier targeted-maze --timeout-seconds 300
python tools/run_test_suite.py --tier targeted-maze-anti-loop --timeout-seconds 300
```

MazeSim is a lightweight synthetic LocalSim-style benchmark for unknown-topology exploration, dead ends, loops, blocked corridors, replanning, and map coverage metrics. The predicted `world_model.json` is built from observations, action results, frontier updates, blocked-edge failures, and replanning traces. `reference_maze.json` is a separate MazeSim spec answer key used only by `comparison_report.json` and validators; `reference_used_for_generation=false` is enforced. It is not an official EAGC runtime, does not use external simulator assets, does not call Qwen, and does not train a model. It complements VirtualHome: VirtualHome stresses household scene graph/action evidence, while MazeSim stresses topology exploration and planning.
...
The maze stress and anti-loop tiers write `world_model.json`, `episode_log.jsonl`, `run_audit.json`, `maze_metrics.json`, `status.json`, `reference_maze.json`, and `comparison_report.json`; anti-loop also writes `anti_loop_report.md` and per-scenario subdirectories with the same predicted/reference artifacts. It adds adversarial synthetic cases for loop lure, comb dead ends, blocked shortcuts, and unreachable goals. Its metrics include repeated states, oscillations, no-progress windows, dead-end reentries, blocked-edge retries, and graceful termination reasons.
...
`dead_end_comb_maze` intentionally causes repeated dead-end discovery and backtracking. The agent path is longer than the shortest path, but oscillation remains zero and the goal is reached. `blocked_shortcut_maze` prioritizes safe recovery over exhaustive map completion; the agent avoids spurious edges while maintaining high topology recall.
...
git clone https://github.com/xavierpuigf/virtualhome.git <external-tools>\virtualhome
$env:VIRTUALHOME_REPO_PATH="<external-tools>\virtualhome"
$env:VIRTUALHOME_SIMULATOR_PATH="<external-tools>\virtualhome_simulator\<actual_exe_name>.exe"
```
...
VirtualHome evidence is optional for the final bundle. Main closed-loop Track 1 evidence is provided by LocalSim and MazeSim. VirtualHome is accepted as final evidence only when `evidence_level=closed_loop_final_evidence`: one continuous episode with reset/add-character once, real Unity image observations, sequential action execution, real `vlm_frame_extraction`, and a predicted `world_model.json` generated from observations/action traces. The VirtualHome scene graph is written separately as `reference_world_model.json` and used only as a local validation answer key in `comparison_report.json`; it is never used to populate the predicted model. Mock, synthetic, replay, or keyframe-only VirtualHome outputs are diagnostics/CI smoke only and are not copied into final `sample_outputs`.

`run_virtualhome_live` and `run_virtualhome_continuous` are fail-fast: they require either `--virtualhome-exe` or `--attach-existing`, validate real captured image files, call the local vision endpoint for `vlm_frame_extraction`, and do not synthesize success if Unity/VirtualHome cannot be reached. The committed `assets/test_sequences/virtualhome_exploration` frames are small synthetic replay fixtures for mock smoke tests, not final VirtualHome evidence.

The VirtualHome continuous mode uses an observation-driven agent policy. The policy selects bounded VirtualHome actions from the current visual extraction, predicted world model, recent events, and available action schema. The harness validates and executes actions and records any fallback.

Strict optional final Windows continuous run:
...
python -m harness.run_virtualhome_replay ^
  ...
  --output-dir outputs/virtualhome_mock_replay ^
...
Package tracked source files plus the harness refresh files:
...
- `tools/package_source.py` creates `dist/source.zip`.
...
- `tools/package_source.py` creates `dist/source.zip` and verifies that outputs, local images except approved fixtures, `.venv-ai2thor`, `source_pack`, zip files, and `__pycache__` are excluded.
...
The generated `submission_bundle/` includes Docker instructions, the prepared source zip, report/disclosure documents, sample `world_model.json`, `episode_log.jsonl`, `run_audit.json`, and `harness_result.json` outputs, and SHA256 checksums. It is a local upload-preparation artifact and is ignored by git.

## Adapter Layout

- `env_adapters/base.py`: stable adapter interface
- `env_adapters/mock_env.py`: current local demo environment
- `env_adapters/official_env.py`: fail-closed placeholder for a future official EAGC runtime adapter
- `executor/action_translator.py`: schema-aware action translation boundary

The rest of the pipeline depends on `BaseEnvAdapter`, so the official adapter can connect through `reset/observe/step/action_schema/capabilities` without rewriting planning, execution, world-model updates, or logging.

The official EAGC runtime/API is still not integrated. Official mode exits non-zero rather than falling back to LocalSim until the official runtime, API, and schema are available.
