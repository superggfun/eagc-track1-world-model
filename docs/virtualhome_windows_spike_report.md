...
- External directories were created outside the EAGC project:
  - `<external-tools>\virtualhome`
  - `<external-tools>\virtualhome_simulator`
- `<external-tools>\virtualhome` exists, but it is currently empty or otherwise not a VirtualHome repository exposing `simulation/unity_simulator/comm_unity.py`.
- `<external-tools>\virtualhome_simulator` exists, but no Windows Unity simulator `.exe` was found.
