...
- `<external-tools>\virtualhome` exists but is not currently a VirtualHome repo/API checkout.
- `<external-tools>\virtualhome_simulator` exists but contains no Windows Unity simulator `.exe`.
