...
```powershell
wsl bash -lc "cd '<repo-root>' && source .venv-ai2thor/bin/activate && python tools/check_ai2thor_rendering_env.py"
wsl bash -lc "cd '<repo-root>' && source .venv-ai2thor/bin/activate && python tools/run_ai2thor_minimal_render_test.py --platform cloud --timeout-seconds 180"
```
